...
// routes/itemInput.js (CommonJS)
'use strict';

const express = require('express');
// Reuse the pooled MySQL connection you already export in other routers
// e.g., your prices/shops routers use: const { pool } = require('../db');
 ...
 *  - itemID
 *  - chainShopID
 *  - channel            (expects 'online'|'physical'; case-insensitive)
 *  - from               (YYYY-MM-DD or ISO string; filters createdAt >= from)
 *  - to                 (YYYY-MM-DD or ISO string; filters createdAt <= to)
...
router.get('/', async (req, res) => {
  try {
    ...
    // Select the exact columns shown in the screenshots
    const sql = `
      SELECT
    ...
    // Light normalization to predictable JS types
